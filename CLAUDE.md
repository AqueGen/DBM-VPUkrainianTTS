...
- `Options.lua` - one checkbox. Switching on applies live; switching off asks for a reload, because `AddRename` has no removal counterpart and its layer is first-writer-wins. `ns.applied` records whether anything was actually registered, so a switch-on in a city neither claims success nor demands a reload, and the panel warns when `DBM.Options.SpecialWarningShortText` is off, which would make the feature invisible.

Kept deliberately absent: per-role wording, a format dropdown, category filters, a slash command, a shortcut to DBM's own bar setting. Each was built and removed - the owner wanted one switch anyone can explain, and role-dependent text could never update without a reload because a rename is baked in for the session.
