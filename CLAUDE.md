...
`text/` turns DBM's on-screen warnings into the same line the pack speaks. It hangs off `DBM:AddRename`, whose layer beats the boss mods' own `RegisterAltSpellName` and loses to the player's Rename button, so nothing a player typed is overwritten. The saved variable is `DBMVPUkrainianTTSDB`, the feature is off by default, and both settings need a `/reload` because `AddRename` has no removal counterpart.
