...
`text/` turns DBM's on-screen warnings into the same line the pack speaks. It hangs off `DBM:AddRename`, whose layer beats the boss mods' own `RegisterAltSpellName` and loses to the player's Rename button, so nothing a player typed is overwritten. The saved variable is `DBMVPUkrainianTTSDB` and the feature is off by default. `AddRename` has no removal counterpart and its layer is first-writer-wins, so only the first switch-on of a session applies live: `ns.applied` records that a rename was registered, `ns.appliedRole` the role it was written for, and `Options.lua` raises a reload prompt for every change that cannot take effect without one, a spec swap that crosses roles included.

`Options.lua` also carries a checkbox for DBM's own `ShortTimerText`, built with `Settings.RegisterProxySetting` so its getter and setter touch `DBM.Options` at call time. A snapshot taken at registration went stale the moment the player used DBM's own panel or switched a DBM profile, which swaps the whole `DBM.Options` table.

A role change also needs a reload, but it only prints a chat line: people swap specs often, and a dialog every time was the first thing the owner complained about. Otherwise a reload is only demanded when renames are already registered. Boss mods are load-on-demand, so switching the feature on in a city registers nothing and needs no reload - the text arrives with the next `DBM*` addon. `Options.lua` and `Commands.lua` say that in chat instead, and both also warn when `DBM.Options.SpecialWarningShortText` is off, which would make the whole feature invisible. `Commands.lua` adds `/uatext`, which toggles the feature and says whether a reload is still needed. A `showTags` boolean from the first version migrates to the `format` string in `ns.DB()`.

- `text/Phrases.lua` - generated by `generation/build_action_text.py` from `ua_table.tsv`. Never edit it; regenerate after touching the table, or the screen and the audio drift apart.
- `text/Actions.lua` - the curated layer: tag plus a purpose-written phrase per voice key, for the keys current content actually uses. It wins over everything else. An optional third element holds per-role wording (`{healer = "...", dps = "..."}`) and is only written for the keys where the role changes what you do - twelve tank and cooldown keys. Everything else is role blind on purpose: a dodge is a dodge for all three roles, and duplicated variants rot.
- `text/Tags.lua` - coarse tag per voice key for older content, paired with the pack's own phrase.
- `text/Engine.lua` - walks `DBM.Mods`, reads `obj.voiceFile` on every special warning and registers the rename. It re-runs on `ADDON_LOADED` for any `DBM*` addon, because boss mods load per zone.

`python generation/check_pack.py` fails if a key in `text/` is missing from the tables or if `Phrases.lua` is out of date. `lua tests/action_text.lua` (Lua 5.1, from the pack root) covers composition, the off switch and the event wiring.
