...
`text/` turns DBM's on-screen warnings into the same line the pack speaks. It hangs off `DBM:AddRename`, whose layer beats the boss mods' own `RegisterAltSpellName` and loses to the player's Rename button, so nothing a player typed is overwritten. The saved variable is `DBMVPUkrainianTTSDB`, and the whole feature is one boolean, off by default.
...
- `text/Actions.lua` - the curated layer: an action tag plus a purpose-written phrase per voice key, for the keys current content uses. A key that is not here falls back to the spoken phrase alone.
...
- `Options.lua` - a checkbox and a format dropdown (`tagphrase`, `tag`, `phrase`; a `showTags` boolean from the first version migrates into it). Switching on applies live; switching off asks for a reload, because `AddRename` has no removal counterpart and its layer is first-writer-wins. `ns.applied` records whether anything was actually registered, so a switch-on in a city neither claims success nor demands a reload, and the panel warns when `DBM.Options.SpecialWarningShortText` is off, which would make the feature invisible.

Kept deliberately absent: per-role wording, category filters, a slash command, a shortcut to DBM's own bar setting. Each was built and removed - the owner wanted one switch anyone can explain, and role-dependent text could never update without a reload because a rename is baked in for the session.
