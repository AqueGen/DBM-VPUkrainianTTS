...
`Options.lua` also carries a checkbox for DBM's own `ShortTimerText`. It is a shortcut to a foreign setting, not a copy: the value is read from `DBM.Options` when the panel registers and written straight back, so DBM's own checkbox stays the source of truth. `Commands.lua` adds `/uatext`, which toggles the feature and says whether a reload is still needed. A `showTags` boolean from the first version migrates to the `format` string in `ns.DB()`.
